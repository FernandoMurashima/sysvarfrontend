...
export class CoresService {
  ...
  list(params?: { search?: string; Codigo?: string; Descricao?: string; Cor?: string; Status?: string; status?: string; ordering?: string; page?: number; page_size?: number }): Observable<Cor[] | any> {
    let httpParams = new HttpParams();
    if (params?.search)    httpParams = httpParams.set('search', params.search);
    if (params?.Codigo)    httpParams = httpParams.set('Codigo', params.Codigo);
    if (params?.Descricao) httpParams = httpParams.set('descricao', params.Descricao);
    if (params?.Cor)       httpParams = httpParams.set('cor', params.Cor);
