...
export class CfopsComponent {
  ...
  private readonly viewPrefsKey = 'sysvar.ui.preferences.cfops';
  selectedItem = signal<Cfop | null>(null);
  ...
  indicadores = computed(() => {
    ...
    return {
      ...
      estoque: rows.filter(i => i.movimenta_estoque).length,
      financeiro: rows.filter(i => i.gera_financeiro).length,
    };
  });
  ...
  onPageSizeChange(v: number) { this.pageSize.set(+v); localStorage.setItem('sysvar.list.cfops.pageSize', String(+v)); this.page.set(1); }
  ...
  selecionarItem(row: Cfop): void { this.selectedItem.set(this.isSelected(row) ? null : row); }
  isSelected(row: Cfop): boolean { return !!this.selectedItem() && this.selectedItem()?.id === row.id; }
  ...
  restoreViewPreference(): void {
    ...
    localStorage.removeItem('sysvar.list.cfops.pageSize');
  ...
  @HostListener('window:sysvar-cfops-toggle-indicators') onToggleIndicatorsEvent(): void { this.toggleIndicators(); }
  @HostListener('window:sysvar-cfops-toggle-filters') onToggleFiltersEvent(): void { this.toggleFilters(); }
  @HostListener('window:sysvar-cfops-restore-view') onRestoreViewEvent(): void { this.restoreViewPreference(); }
  ...
  private loadColumnsPreference(): void {
    const size = Number(localStorage.getItem('sysvar.list.cfops.pageSize'));
